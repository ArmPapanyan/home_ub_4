...
class TestPositive:
   ...
   def test_step2(self, make_folders, clear_folders, make_files, start_time):
       res1 = ssh_checkout(data["ip"], data["user"], data["passwd"], "cd {};"
           " 7z a {}/arx2".format(data["FOLDER_TST"], data["FOLDER_OUT"]), "Everything is Ok")
   ...
   def test_step3(self, clear_folders, make_files, start_time):
       ...
       res.append(ssh_checkout(data["ip"], data["user"], data["passwd"], "cd {}; 7z a "
           "{}/arx2".format(data["FOLDER_TST"], data["FOLDER_OUT"]), "Everything is Ok"))
       res.append(ssh_checkout(data["ip"], data["user"], data["passwd"], "cd {}; 7z e "
           "arx2.7z -o{} -y".format(data["FOLDER_OUT"], data["FOLDER_1"]), "Everything is Ok"))
       for item in make_files:
           res.append(ssh_checkout(data["ip"], data["user"], data["passwd"], "ls {}".format(data["FOLDER_1"]), item))
   ...
   def test_step5(self, start_time):
       ...
       assert ssh_checkout(data["ip"], data["user"], data["passwd"], "cd {}; 7z u"
           " arx2.7z".format(data["FOLDER_TST"]), "Everything is Ok"), "test5 FAIL"
   ...
   def test_step6(self, clear_folders, make_files, start_time):
       ...
       for item in make_files:
           ...
           " arx2.7z".format(data["FOLDER_OUT"], data["FOLDER_1"]), item))
   ...
   def test_step7(self, clear_folders, make_files, make_subfolder, start_time):
       res = []
       res.append(ssh_checkout(data["ip"], data["user"], data["passwd"], "cd {}; 7z a "
           "{}/arx".format(data["FOLDER_TST"], data["FOLDER_OUT"]), "Everything is Ok"))
       res.append(ssh_checkout(data["ip"], data["user"], data["passwd"], "cd {};"
           " 7z x arx.7z -o{} -y".format(data["FOLDER_OUT"], data["FOLDER_2"]), "Everything is Ok"))


       for item in make_files:
           res.append(ssh_checkout(data["ip"], data["user"], data["passwd"], "ls {}".format(data["FOLDER_2"]), item))


       res.append(ssh_checkout(data["ip"], data["user"], data["passwd"], "ls {}".format(data["FOLDER_2"]),
                               make_subfolder[0]))
       res.append(ssh_checkout(data["ip"], data["user"], data["passwd"], "ls {}/{}".format(data["FOLDER_2"],
   ...
   def test_step9(self, clear_folders, make_files, start_time):
       ...
       res = []
       for item in make_files:
           res.append(ssh_checkout(data["ip"], data["user"], data["passwd"], "cd {}; 7z h {}".format(data["FOLDER_TST"],
                   item), "Everything is Ok"))
           hash = ssh_getout(data["ip"], data["user"], data["passwd"], "cd {}; "
                                                                   "crc32 {}".format(data["FOLDER_TST"], item)).upper()
           res.append(ssh_checkout(data["ip"], data["user"], data["passwd"], "cd {}; 7z h {}".format(data["FOLDER_TST"],
                                                                                                     item), hash))
